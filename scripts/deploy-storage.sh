...
init_script
prepare_kubectl_access
if [[ ! -f "${K8S_BASE_DIR}/storage/minio/secret.yaml" ]]; then
  echo "Secret file not found: ${K8S_BASE_DIR}/storage/minio/secret.yaml"
  exit 1
fi
echo "Storage secret files look ready"
echo "Rendering manifests from ${K8S_BASE_DIR}/storage"
kubectl_cmd kustomize "${K8S_BASE_DIR}/storage" >/dev/null
echo "Applying storage manifests"
kubectl_cmd apply -k "${K8S_BASE_DIR}/storage"
echo "Waiting for deployment/minio"
kubectl_cmd rollout status deployment/minio --timeout="${WAIT_TIMEOUT:-180s}"
echo "Storage is deployed"
