...
check_observability_secrets() {
  local secret_file
  for secret_file in \
    "${K8S_BASE_DIR}/observability/grafana/admin-secret.yaml" \
    "${K8S_BASE_DIR}/observability/grafana/db-secret.yaml"; do
...
prepare_kubectl_access
check_observability_secrets
echo "Observability secret files look ready"
echo "Rendering manifests from ${K8S_BASE_DIR}/observability"
kubectl_cmd kustomize "${K8S_BASE_DIR}/observability" >/dev/null
echo "Applying observability manifests"
kubectl_cmd apply -k "${K8S_BASE_DIR}/observability"
for deployment_name in prometheus mimir pyroscope tempo loki alloy-logs grafana; do
  echo "Waiting for deployment/${deployment_name}"
  kubectl_cmd rollout status "deployment/${deployment_name}" --timeout="${WAIT_TIMEOUT:-180s}"
done
echo "Observability is deployed"
